...
## Named limitation for §3.7 (added 2026-06-02, after 11 controls)

> Report-ready prose, drop-in for `outputs/report/research_report.md` §3.7 once the control sample is integrated. Refines the existing **"Writeup availability bias"** and **"No external original solutions for comparison"** limitations rather than replacing them.

**Documentation self-selection in the control sample.** The non-winning controls are filtered twice, not once: an entry qualifies only if it (a) finished in the rank-4–30 band (an outcome filter selecting the top ~1% of finishers) *and* (b) published a public writeup (a documentation filter). The second filter is not independent of competitor identity. Among lower-ranked entries especially, writeups are produced almost exclusively by *recurring documenters* — competitors who publish a solution every season as a matter of habit or personal brand, regardless of where they place (Ravi Ramakrishnan, for example, has published writeups for finishes ranging from rank 2 to rank 28). Because habitual documentation correlates strongly with being a serial elite competitor, conditioning on writeup-existence structurally over-samples a small recurring guild. This has a direct consequence for the study's author-overlap statistic (5 of 11 controls share an author with the winners corpus, 45%): the overlap is partly *instrument-induced*. If public writeups at these ranks originate from a pool of only ~10–15 habitual documenters, then any two writeup-filtered samples will overlap heavily almost irrespective of the underlying leaderboard composition. The defensible reading is therefore that the *documented* top of the leaderboard is a small recurring community — not, more strongly, that the *winning* population is a small clique. A genuine residual survives this deflation: documentation explains why these competitors are *visible*, but not why they repeatedly finish near the top rather than mid-pack, so the recurrence of high finishes among documenters remains evidence of skill persistence, observed through a documentation keyhole. Two practical corollaries follow. First, sampling deeper into the leaderboard does not relax this filter — if anything it tightens it, since casual competitors document their solutions even less frequently at lower ranks — so a broader rank range would not yield a more representative control set. Escaping the filter requires changing the instrument itself (for example, public notebooks sampled by leaderboard quartile, or solution descriptions from rank-100+ discussion threads contributed by non-writeup-authors). Second, the winners corpus is subject to the identical documentation filter, so the winner-versus-control comparison remains internally consistent (documented-finisher versus documented-finisher); the limitation is that *neither* sample represents a typical strong competitor.
