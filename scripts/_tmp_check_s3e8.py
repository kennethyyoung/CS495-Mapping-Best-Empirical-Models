...
fields = ["finish_rank", "code_url", "writeup_detail", "missing_data_strategy",
          "encoding_strategy", "scaling", "cv_strategy", "ensemble_method",
          "fe_techniques", "hyperparameter_tuning", "original_data_usage"]

for r in range(2, ws.max_row + 1):
    v = ws.cell(row=r, column=ref_col).value
    if str(v) == "playground-series-s3e1":
        print(f"ref: {v}")
        for f in fields:
            if f in headers:
