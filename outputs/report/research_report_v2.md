...
**Near-winning controls.** To test whether feature engineering distinguishes winners from strong non-winners (Section 4.4), a second, smaller corpus of 11 *near-winning controls* was collected. A control is a rank 4–25 finisher, from a subset of the same competitions as the winners corpus, who published a writeup and/or notebook detailed enough to code on the feature-engineering taxonomy. Controls were collected through a feasibility pilot followed by targeted expansion across paradigms and eras; the rank-4–25 band was chosen as the closest documented non-winning tier (rank 1–3 entries are effectively co-winners, frequently within a 0.00001 leaderboard margin, while documented solutions below rank ~25 become scarce). Controls were coded only on the Pass 3 feature-engineering taxonomy (Section 3.4), not on the full Pass 1/Pass 2 schema, because the comparison of interest is feature-engineering intensity and composition. An important property of this control pool — that it is itself drawn from the documented elite and overlaps the winners corpus in authorship — is treated as a finding (Section 4.5) and a limitation (Section 3.8) rather than a nuisance.
...
### 4.5 Technique Volume Does Not Distinguish Winners; Learned Features Do
...
The one exception is decisive in its narrowness. **Learned and advanced feature techniques (Group G — autoencoder latents, genetic-programming features, PCA/SVD components) are the only family in which winners exceed controls** (16% vs 9%), and this difference is concentrated in heavyweight ensemble-stacking entries, where the heavyweight winners examined used at least one Group G technique while their rank-matched controls used none. What distinguishes a winning tabular solution from a near-winning one is thus not *more* feature engineering but a *qualitatively different kind* — learned, derived features rather than additional hand-crafted ones.

This null contrast on volume should be read together with the composition of the control pool (Section 4.6) and the documentation-self-selection limitation (Section 3.8): because the controls are themselves a documented near-winning elite — and in 5 of 11 cases share an author with the winners corpus — the comparison establishes that feature-engineering intensity does not separate winners from *near-winners*, not that it fails to separate them from typical competitors.

[Figure 5 — FE family prevalence, winners vs near-winning controls. Source: `analysis/figures/report/fig5_fe_prevalence_winners_vs_controls.png`]

### 4.6 The Documented Top Is a Recurring Guild

The winner–control comparison (Section 4.5) showed that technique inventory does not separate winners from near-winners. The composition of the corpus points to a structural answer to what does: the documented top of these leaderboards is a small, recurring pool of highly skilled, socially connected competitors.
...
[Figure 6 — author centrality. Source: `analysis/figures/phase5/phase5_41_author_centrality.png`]
[Figure 7 — cdeotte timeline. Source: `analysis/figures/phase5/phase5_42_cdeotte_timeline.png`]

### 4.7 Paradigm by Era

[Figure 8 — Paradigm composition by era. Source: `analysis/figures/phase5/phase5_22_paradigm_by_era.png`]
...
[Figure 9 — n_rows by paradigm, log scale. Source: `analysis/figures/phase5/phase5_24_paradigm_n_rows.png`]
...
[Figure 11 — Photo-finish rate by paradigm. Source: `analysis/figures/phase5/phase5_23_paradigm_photofinish.png`]

### 4.9 Corpus Characteristics

*(The centrality archetypes and the cdeotte timeline formerly reported here are presented in Section 4.6, which they now anchor.)*
...
[Figure 12 — Canonized-technique adoption frequency. Source: `analysis/figures/phase5/phase5_33_canonized_techniques.png`]

[Figure 13 — Citations by origination score. Source: `analysis/figures/phase5/phase5_34_citations_origination.png`]
