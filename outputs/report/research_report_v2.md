...
This study is a meta-analysis of winning solutions in Kaggle tabular machine-learning competitions, asking what actually distinguishes a winning approach. We coded 45 top-place solutions from competitions spanning February 2022 to April 2026 across a structured field schema, a qualitative origination-and-attribution pass, and a 53-technique feature-engineering taxonomy, and compared them against 11 near-winning (rank 4–25) controls. Four findings emerge. First, winning solutions cluster into a small set of recurring paradigms dominated by ensemble stacking (62% of wins). Second, winning feature engineering is a small shared core — target encoding plus a handful of interaction and encoding techniques — atop a long bespoke tail, with the typical winner using only a few of their own techniques (a median of four among well-documented entries, lower where documentation is thin). Third, feature-engineering *volume* does not cleanly distinguish winners from near-winning controls — an aggregate comparison favors the controls but is confounded by competition mix, while a within-competition paired comparison reverses it (winners higher in 7 of 11 pairs), with neither result significant at n = 11; the more consistent signal is learned/advanced features, which favor winners and never controls. Fourth, the documented top is a small recurring guild: 5 of 11 near-winning controls share an author with the winners corpus, and technique knowledge propagates through a few central figures (siukeitin and paddykb as pure technique-sources who never themselves win; cdeotte and others as dominant-and-cited hubs). A blind re-code of a stratified subsample shows substantial agreement at the technique-family level (Cohen's κ = 0.65), though lower at the individual-technique level, supporting analysis at the family level. All claims are scoped by a documentation-self-selection limitation: the corpus represents the *documented* elite, not the typical competitor. The study contributes an empirical typology of winning strategies, a reliability-validated feature-engineering taxonomy, the small-core and learned-feature-discriminator findings, a characterization of the documented top as a recurring guild, and a documented multi-pass coding methodology with full audit trail.
...
The study covers tabular Kaggle competitions concluded between February 2022 and April 2026. The 45 winner entries include Playground Series Seasons 3 through 6, Tabular Playground Series competitions from 2022, and one Featured competition (ICR — Identifying Age-Related Conditions, August 2023). The primary unit of analysis is one winning solution per competition, drawn from whichever top-three finisher published the most complete writeup. The study additionally collected 11 **near-winning controls** — rank 4–25 finishers from a subset of the same competitions who published a codeable solution — to test whether feature engineering distinguishes winners from strong non-winners (Sections 3.2, 4.4). Excluded from scope are: time series and forecasting competitions, image and audio competitions, natural language processing competitions, multi-table join competitions where the winning solution did not first collapse to a single flat file, and the November 2022 TPS meta-blending competition (which involved no feature data).

### 1.6 Report Outline

Section 2 reviews related work in tabular machine learning benchmarking, competition-as-method research, and knowledge propagation in open communities, positioning the gap this study addresses. Section 3 documents the research design, data collection (winners and controls), the multi-pass coding methodology including the feature-engineering taxonomy, the audit and corrections workflow, analytical methods, the coding-reliability check, tools, and limitations. Section 4 reports findings: an overview of the central result (4.1), the four-paradigm typology (4.2) and coupling evidence (4.3), the feature-engineering prevalence map (4.4), the winner–control comparison (4.5), the community/guild structure (4.6), the era shift (4.7), and corpus cross-tabulations and characteristics (4.8–4.9). Section 5 discusses and interprets the findings; Section 6 lists references.
...
**Near-winning controls.** To test whether feature engineering distinguishes winners from strong non-winners (Section 4.5), a second, smaller corpus of 11 *near-winning controls* was collected. A control is a rank 4–25 finisher, from a subset of the same competitions as the winners corpus, who published a writeup and/or notebook detailed enough to code on the feature-engineering taxonomy. Controls were collected through a feasibility pilot followed by targeted expansion across paradigms and eras; the rank-4–25 band was chosen as the closest documented non-winning tier (rank 1–3 entries are effectively co-winners, frequently within a 0.00001 leaderboard margin, while documented solutions below rank ~25 become scarce). Controls were coded only on the Pass 3 feature-engineering taxonomy (Section 3.4), not on the full Pass 1/Pass 2 schema, because the comparison of interest is feature-engineering intensity and composition. An important property of this control pool — that it is itself drawn from the documented elite and overlaps the winners corpus in authorship — is treated as a finding (Section 4.6) and a limitation (Section 3.8) rather than a nuisance.
...
**Design.** An independent blind re-code was conducted on a stratified 12-entry sample (≈ 21% of the combined 56-entry winners-plus-controls corpus), drawn to span eras (one each from the 2022 Tabular Playground series and Seasons 3–6) and source-confidence levels, and mixing eight winners with four controls. The re-coder worked from the original notebooks and writeups plus the schema definitions only — blind to the primary codings, the coding notes, and the data sheets. Agreement was then computed between the blind re-code and the primary codings.
...
**Single-researcher coding, with AI assistance.** All Pass 1 schema field values and Pass 2 paradigm assignments were coded by a single researcher, and the Pass 3 feature-engineering codings were produced with AI assistance under researcher direction. For the Pass 3 taxonomy, a formal reliability check against an independent blind re-code of a stratified subsample was performed (Section 3.6); agreement was substantial at the family level (κ = 0.65) and only moderate at full 53-column resolution (κ = 0.59), which is why feature-engineering results are reported at the group level. The single-coder concern is only partially mitigated for Pass 1 and Pass 2, which were not independently re-coded: for Pass 1 a self-audit was conducted on a random 20% sample after a one-week gap, and the data-quality audit (Section 3.4) served as a structured second-pass check that surfaced 17 cell-level corrections.
...
Feature engineering is widely held to be the decisive craft in tabular machine learning, so one might expect the winners of tabular competitions to be distinguished by especially extensive or sophisticated feature work. The corpus does not bear this out. The results below establish, in turn, what winning solutions look like structurally (Section 4.2, a four-paradigm typology dominated by ensemble stacking; Section 4.3, the constraint-to-strategy couplings that survive testing); that their feature engineering is a small shared core rather than an elaborate practice (Section 4.4); that feature-engineering volume does not cleanly separate them from strong non-winners — the comparison is sensitive to framing, and only learned features show a consistent winner-favoring signal (Section 4.5); that the documented top is a small recurring guild distinguished more by who competes than by technique inventory (Section 4.6); how the field has shifted toward very large ensembles and LLM-assisted workflows in the most recent season (Section 4.7); and the descriptive composition of the corpus (Sections 4.8–4.9). The recurring theme is that *winning is less about feature-engineering volume than the folklore suggests, and more about paradigm fit, a small set of high-leverage techniques, and membership in a small skilled community.*
