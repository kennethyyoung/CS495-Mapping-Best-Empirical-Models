...
**Structurally omitted fields.** Two fields — `missing_data_strategy` and `scaling` — had low rates of active technique usage but high overall completeness when `none` (explicit non-use) is counted as a valid answer. For `missing_data_strategy`, 42 of 45 entries (93%) have a known value: 35 explicitly recorded no treatment (`none` or `not_applicable`) and 7 used an active technique. Only 3 entries are genuinely unknown (`not_described`). The near-zero imputation rate is a structural property of the dataset: 80% of entries are synthetic Playground Series competitions generated without missing values. For `scaling`, 29 of 45 entries (64%) have a known value: 15 explicitly recorded no scaling and 14 used an active method. The remaining 16 entries (36%) are genuinely unknown (`not_described`) — the author did not mention scaling at all, likely because tree-based models are scale-invariant and the decision felt unremarkable. Both fields were excluded from flowchart decision nodes: `missing_data_strategy` because the dominant answer is unambiguously "no treatment needed," and `scaling` because 36% remains unknown.

**Genuinely sparse field.** `distribution_shift` had only 13 of 45 entries (29%) with a TRUE/FALSE value; the remaining 32 (71%) were `not_described`. Unlike the above two fields, this sparsity reflects an actual documentation gap — solution writeups rarely discuss whether train/test distributions differ. This field was dropped as a candidate decision node input.
...
**Field completeness.** Across the 36 schema columns, field-level completeness — defined as the proportion of entries with a non-null, non-`not_described` value — varied widely. Pipeline decision fields with high completeness included: `missing_data_strategy` (93%), `cv_strategy` (91%), `ensemble_method` (89%), `fe_techniques` (93%), and `encoding_strategy` (87%). `scaling` reached 64%. The only genuinely sparse field was `distribution_shift` (29%), where 71% of entries were `not_described` — reflecting a documentation gap in solution writeups. As described in Section 3.7, the low active-technique usage rates for `missing_data_strategy` and `scaling` are structural (most GBM-based solutions neither impute nor scale) rather than collection failures.
