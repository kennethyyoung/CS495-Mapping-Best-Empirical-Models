...
\statbox{5 of 6}{folklore couplings fail}{BCRed}%
\hspace*{\fill}
...
\textbf{Does elaborate feature engineering win tabular Kaggle competitions?} We coded
\textbf{45 winning solutions} (2022--2026) and \textbf{11 near-winning controls} across a
53-technique taxonomy. Three findings:
\begin{itemize}[leftmargin=1.4em,itemsep=4pt,topsep=4pt]
  ...
  \item Winning feature engineering is a \textbf{small shared core} --- it does \emph{not cleanly} separate winners from near-winners ($n=11$, n.s.).
...
\vspace{0.5em}
\textbf{Keywords:} \textit{meta-analysis, Kaggle, tabular ML, feature engineering, ensembling, content analysis}
\end{posterblock}
...
on fixed data --- never winners against strong non-winners. We code both.

\vspace{0.5em}
\textbf{Research Questions}
\begin{itemize}[leftmargin=1.4em,itemsep=5pt,topsep=4pt]
  \item \textbf{RQ1} --- What strategy \emph{paradigms} define winning, and do claimed constraint$\rightarrow$strategy rules hold?
  \item \textbf{RQ2} --- Which FE techniques do winners use, and how much?
...
Strategies are not evenly spread: \textbf{ensemble stacking} takes most wins, and
gradient-boosted trees are the dominant model in \textbf{35 of 45} entries.

\vspace{0.4em}
\includegraphics[width=\linewidth]{fig01_paradigm_distribution}
\figcap{Figure 1. Paradigm distribution across the 45 winning solutions.}
\end{posterblock}
...
\node[io] (corpus) {{\fontsize{15}{17}\selectfont\bfseries Corpus}\\[4pt]45 winners\\+ 11 controls};
\node[box, right=of corpus] (p1) {{\fontsize{15}{17}\selectfont\bfseries\color{BCBlue}Pass 1}\\[4pt]structured\\field coding};
\node[box, right=of p1] (p2) {{\fontsize{15}{17}\selectfont\bfseries\color{BCBlue}Pass 2}\\[4pt]paradigm \&\\attribution};
\node[box, right=of p2] (p3) {{\fontsize{15}{17}\selectfont\bfseries\color{BCBlue}Pass 3}\\[4pt]FE taxonomy\\(53 techniques)};
\node[result, right=of p3] (out) {{\fontsize{15}{17}\selectfont\bfseries Analyses}\\[4pt]typology\\winner--control\\couplings};
\draw[arr] (corpus) -- (p1);
\draw[arr] (p1) -- (p2);
\draw[arr] (p2) -- (p3);
\draw[arr] (p3) -- (out);
...
\node[text=BCRed, font=\fontsize{11}{13}\selectfont\bfseries, below=5mm of p2.south]
  {blind intra-rater re-code \textbullet\ $\kappa = 0.65$};
\end{tikzpicture}%
}}
\figcap{Figure 2. Multi-pass coding workflow with a near-winner control group and reliability check.}
\end{posterblock}
...
competition --- the comparison that isolates what distinguishes \emph{winning} from
merely strong play.
\end{callout}
\end{posterblock}
...
\textbf{FE does not separate winners from near-winners} ($n=11$, not significant); only
\emph{learned} features lean winner.

\vspace{0.4em}
\centerline{\includegraphics[width=0.86\linewidth]{fig05_fe_prevalence_winners_vs_controls}}
\figcap{Figure 3. FE-family prevalence, winners vs.\ near-winning controls.}

\vspace{0.5em}
\textbf{5 of 6 folklore couplings fail} --- only linear-stacker dominance holds.

\vspace{0.4em}
\centerline{\includegraphics[width=0.86\linewidth]{fig02_coupling_evidence}}
...
The author thanks \textbf{Dr.~Pedro Albuquerque} for his guidance throughout CS\,495, and
the Bellevue College Computer Science program for supporting this capstone.

\vspace{0.5em}
\textbf{Contact:} kenneth.young@bellevuecollege.edu\\
\textbf{Code / Repository:} \textit{[github.com/\ldots\, --- add URL]}
\end{posterblock}
